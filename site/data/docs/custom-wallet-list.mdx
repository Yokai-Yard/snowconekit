...
```tsx
import { connectorsForWallets, wallet } from '@snowcone/snowconekit';
...
```tsx
import { wallet } from '@snowcone/snowconekit';
...
const connectors = connectorsForWallets([
  {
    ...
    wallets: [
      ...
      wallet.coinbase({ appName: 'My SnowConeKit App', chains }),
      wallet.metaMask({ chains }),
...
const connectors = connectorsForWallets([
  ...
  },
  {
    ...
    wallets: [
      wallet.walletConnect({ chains }),
      wallet.coinbase({ chains, appName: 'My SnowConeKit App' }),
    ],
  },
]);
```
