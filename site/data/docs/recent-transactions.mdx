...
You can opt in to displaying recent transactions within SnowConeKit's account modal. Note that all transactions are kept in local storage and must be manually registered with SnowConeKit in order to be displayed.
...
To use this feature, first enable the `showRecentTransactions` option on `SnowConeKitProvider`.

```tsx
import { SnowConeKitProvider } from '@sirbenchalot757/snowconekit';
...
Transactions can then be registered with SnowConeKit using the `useAddRecentTransaction` hook.

```tsx
import { useAddRecentTransaction } from '@sirbenchalot757/snowconekit';
...
Once a transaction has been registered with SnowConeKit, its status will be updated upon completion.
