...
The `chains` prop on `SnowConeKitProvider` defines which chains are available for the user to select.

SnowConeKit is designed to integrate with [wagmi’s `chain` object](https://wagmi.sh/docs/constants/chains#chain) which currently provides the following chains:
